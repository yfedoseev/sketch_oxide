...
describe('BloomFilter', () => {
  ...
  describe('merge', () => {
    it('should merge two compatible filters', () => {
      const bf1 = new BloomFilter(10000, 0.01)
      const bf2 = new BloomFilter(10000, 0.01)

      bf1.insert(Buffer.from('first'))
      bf2.insert(Buffer.from('second'))

      bf1.merge(bf2)

      expect(bf1.contains(Buffer.from('first'))).toBe(true)
      expect(bf1.contains(Buffer.from('second'))).toBe(true)

      bf1.dispose?.()
      bf2.dispose?.()
    })

    it('should throw on merge with different n', () => {
      const bf1 = new BloomFilter(10000, 0.01)
      const bf2 = new BloomFilter(5000, 0.01)
    ...
    it('should throw on merge with different fpr', () => {
      const bf1 = new BloomFilter(10000, 0.01)
      const bf2 = new BloomFilter(10000, 0.05)
    ...
    it('should merge filters with identical data', () => {
      const bf1 = new BloomFilter(10000, 0.01)
      const bf2 = new BloomFilter(10000, 0.01)
      ...
      for (const item of items) {
        bf1.insert(Buffer.from(item))
        bf2.insert(Buffer.from(item))
      }

      bf1.merge(bf2)

      for (const item of items) {
        expect(bf1.contains(Buffer.from(item))).toBe(true)
      }
    ...
    it('should merge filters with overlapping data', () => {
      const bf1 = new BloomFilter(10000, 0.01)
      const bf2 = new BloomFilter(10000, 0.01)

      for (let i = 0; i < 500; i++) {
        bf1.insert(Buffer.from(`item-${i}`))
      }

      for (let i = 250; i < 750; i++) {
        bf2.insert(Buffer.from(`item-${i}`))
      }
      ...
      for (let i = 0; i < 750; i++) {
        expect(bf1.contains(Buffer.from(`item-${i}`))).toBe(true)
      }
